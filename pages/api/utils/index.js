...
export const createOneBucket = async (bucketName) => {
  client.makeBucket(`smartreportzFloats${bucketName}`, 'us-east-1', function (err) {
...
export const getOneLinkMinio = async ({ fileName }) => {
  const data = await client.presignedUrl('GET', 'Floats', fileName, 24 * 60 * 60)
  return { success: true, message: data }
}
// CREATE ONE FILE
export const deleteOneFileMinio = async ({ fileName }) => {
  client.removeObject('Floats', fileName, function (err) {
    if (err) {
      return {
        success: false,
